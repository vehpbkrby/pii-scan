...
#
# Kafka, RabbitMQ, NATS, FileLog и S3Queue — потоковые движки. Прямой SELECT
# из них читает сообщения через консьюмер-группу и фиксирует offset: сами
# сообщения остаются в топике до истечения retention, но материализованное
# представление, наливающее целевую таблицу, их уже не увидит. Откатывается
# только ручным сбросом offset, поэтому такие таблицы не сканируются.
# ClickHouse запрещает прямой SELECT и сам (ошибка 620), но запрет снимается
# настройкой stream_like_engine_allow_direct_select — полагаться на него
# нельзя.
...
class ScanOptions:
    ...
    # Вклад совпадения по имени колонки. Понижать осторожно: коэффициент
    # снимает уверенность и с находок, подтверждённых значениями. Чтобы
    # убрать из отчёта то, за чем нет ничего кроме названия поля, есть
    # confirmed_only — он для этого и сделан.
    name_boost: float = 0.35
    # true — в отчёты попадают только находки, подтверждённые содержимым.
    # Упрощённый отчёт: без полей, весь вывод по которым держится на имени.
