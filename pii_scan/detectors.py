...
# Значения-заглушки: способ записать «здесь ничего нет», когда колонка
# объявлена NOT NULL или приложение не умеет писать NULL. Считать их доводом
# против ПДн неверно — это отсутствие значения, а не другое значение.
#
# «0», «нет», «х» сюда намеренно не включены: они бывают настоящими
# значениями, а ошибка в эту сторону завышает долю совпадений.
_PLACEHOLDERS = frozenset([
    "-", "--", "---", "—", "–", "?", "??", "???", ".", "..", "...",
    "n/a", "n\\a", "na", "none", "null", "nil", "undefined", "unknown",
    "empty", "no data", "not set", "not specified",
    "н/д", "нд", "н\\д", "б/н", "без номера",
    "не указан", "не указана", "не указано", "не указаны",
    "не заполнено", "не заполнен", "не задано", "не определено",
    "нет данных", "данные отсутствуют", "отсутствует", "отсутствуют",
    "неизвестно", "неизвестен", "пусто", "прочерк",
])


def is_placeholder(value: str) -> bool:
    """Значение, которым записали отсутствие значения."""
    v = " ".join(str(value).split()).strip(" .:;،,").lower()
    if not v:
        return True
