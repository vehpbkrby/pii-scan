...
#
# chmod по /app обязателен: COPY переносит права файлов из контекста сборки
# как есть. На сервере с umask 077 (обычное дело для укреплённых сборок)
# внутрь образа попадут файлы с режимом 600, принадлежащие root, и
# контейнер, запущенный под любым другим пользователем, их не прочитает.
#
# /opt/venv трогать не нужно и вредно: он создан внутри сборки с обычными
# правами, а рекурсивный chmod по нему продублировал бы в новый слой все
# зависимости — образ вырос бы вдвое.
